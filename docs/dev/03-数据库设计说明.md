...
数据库用于保存旅游管理系统的核心业务数据，包括用户、景点、线路、订单、评论、收藏和公告。设计原则是结构清晰、字段够用、约束明确，避免为了课程设计引入过多复杂关系。
...
erDiagram
    ...
    SCENIC_SPOT ||--o{ COMMENT : has
    TOUR_ROUTE ||--o{ COMMENT : has
...
CREATE TABLE scenic_spot (
  ...
  KEY idx_scenic_spot_category (category)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
```
...
CREATE TABLE tour_route (
  ...
  KEY idx_tour_route_status_time (status, departure_time)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
```
...
| `comment.status` | PENDING、APPROVED、REJECTED | 评论审核状态。 |
